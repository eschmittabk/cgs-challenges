...
bool Level::Load(std::string levelName, int* playerX, int* playerY, int* slenderX, int* slenderY)
{
	levelName.insert(0, "../");
	ifstream levelFile;
	...
	else
	{
		...
		// Read level
		m_pLevelData = new char[(long long)m_width * (long long)m_height];
		levelFile.read(m_pLevelData, (long long)m_width * (long long)m_height);
		
		// Convert level
		bool anyWarnings = ConvertLevel(playerX, playerY, slenderX, slenderY);
		if (anyWarnings)
		{
...
bool Level::ConvertLevel(int* playerX, int* playerY, int* slenderX, int* slenderY)
{
	...
	for (int y = 0; y < m_height; ++y)
	{
		for (int x = 0; x < m_width; ++x)
		{
			...
			case '!':
				m_pLevelData[index] = ' ';
				if (slenderX != nullptr && slenderY != nullptr)
				{
					*slenderX = x;
					*slenderY = y;
				}
...
PlacableActor* Level::CheckActors(int x, int y) //same as UpdateActors but with no movement
{
	PlacableActor* collidedWithSlender = nullptr;
	...
	for (auto actor = m_pActors.begin(); actor != m_pActors.end(); ++actor)
	{
		...
		{
			// should only be able to collide with one actor
			assert(collidedWithSlender == nullptr);
			collidedWithSlender = (*actor);
		}
	}

	return collidedWithSlender;
}